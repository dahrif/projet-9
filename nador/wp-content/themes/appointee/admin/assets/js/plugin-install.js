jQuery(document).ready(function($) {
    'use strict';
    var this_obj = appointee_companion_install;
    ...
    $(document).on('click', '.action-watch', function(event) {
        ...
        if(action_id){
            $.ajax({
                ...
                type: 'POST',
                data: {action: 'appointee_update_rec_acts', action_id:action_id},
                success:function(data) {
    ...
    $(document).on('click', '.webriti-customizer-notification-dismiss', function(event) {
        ...
        var sdata = {};
        if(slug){
            sdata.action = 'appointee_hide_customizer_notice';
            sdata.appointee_plugin_slug = slug;
        }else{
            sdata.action = 'appointee_hide_customizer_companion_notice';
        }
